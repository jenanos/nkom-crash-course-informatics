...
## Del G: Krypter og dekrypter en tekstfil
...
```bash
echo "Gratulerer! Du har dekryptert den hemmelige meldingen." > hemmelig.txt
```

Krypter filen:

```bash
openssl enc -aes-256-cbc -salt -pbkdf2 -in hemmelig.txt -out hemmelig.txt.enc
```

Velg et enkelt passord du husker under øvelsen.
...
```bash
openssl enc -d -aes-256-cbc -pbkdf2 -in hemmelig.txt.enc -out hemmelig.txt
```
...
Dette viser at kommandoer ofte har:
- en inputfil
- en operasjon
- en outputfil
